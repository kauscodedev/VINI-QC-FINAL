"""Behavioral Judge — Resolution & Product Accuracy (needs full tool events to check claims)."""
...
SYSTEM_PROMPT = load_prompt("behavior_resolution_accuracy")


async def judge_behavior_resolution_accuracy(
    ctx: ContextJSON,
    ...
    logger.info(f"behavior_resolution_accuracy: score={result.score} na={result.score_na} issues={len(result.issues)}")
    return result
