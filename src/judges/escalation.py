"""Escalation & Transfer Handling Judge — transfer summaries, error handling, complaint empathy."""
...
SYSTEM_PROMPT = load_prompt("judge_escalation")
...
async def judge_escalation(
    ...
    logger.info(f"escalation: score={result.score} na={result.score_na} issues={len(result.issues)}")
    return result
