"""Behavioral Judge — Conversation Management (empathy, pacing, no loops, style adaptation)."""
...
SYSTEM_PROMPT = load_prompt("behavior_conversation_management")


async def judge_behavior_conversation_management(
    ctx: ContextJSON,
    ...
    logger.info(f"behavior_conversation_management: score={result.score} na={result.score_na} issues={len(result.issues)}")
    return result
