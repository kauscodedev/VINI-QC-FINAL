"""Behavioral Judge — Opening & Tone (did the agent land the first 15 seconds warmly)."""
...
SYSTEM_PROMPT = load_prompt("behavior_opening_tone")


async def judge_behavior_opening_tone(
    ctx: ContextJSON,
    ...
    logger.info(f"behavior_opening_tone: score={result.score} na={result.score_na} issues={len(result.issues)}")
    return result
